...
namespace mvrp {
    ...
    void SPSolver::print_report(int sols_found, int discarded_prc, int discarded_infeasible, int discarded_generated,
        ...
        out << "\t\t\t\t" << discarded_prc << " columns were discarded because they have positive reduced cost."
            << std::endl;
        out << "\t\t\t\t" << discarded_infeasible
            << " columns were discarded because they're infeasible wrt capacity constraints." << std::endl;
        out << "\t\t\t\t" << discarded_generated
            << " columns were discarded because they had already been generated in this iteration." << std::endl;
        out << "\t\t\t\t" << discarded_in_pool
            << " columns were discarded because they were already in the columns pool." << std::endl;
    }

    std::pair<int, ColumnOrigin>
    SPSolver::solve(ColumnPool &node_pool, std::shared_ptr<ColumnPool> global_pool, bool try_elementary,
                    double &max_time_spent_by_exact_solver) const {
        ...
        if(prob->params.try_reduced_labelling) {
            ...
            if(valid_sols.size() > 0) {
                for(const auto &s : valid_sols) {
                    Column col(prob, s,
                               "labelling on the " + std::to_string(percentage - pct_increment) + "-reduced graph",
                               ColumnOrigin::SPPRC_RED);
                    node_pool.push_back(col);
                    global_pool->push_back(col);
                }

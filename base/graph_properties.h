...
public:
    ...
    GraphProperties(const PortDuals port_duals = PortDuals(),
                    const PortDuals old_port_duals = PortDuals(),
                    const float vc_dual = 0,
                    const int pu_upper_bound = 0,
                    const int de_upper_bound = 0) : port_duals(port_duals), old_port_duals(old_port_duals), vc_dual(vc_dual), pu_upper_bound(pu_upper_bound), de_upper_bound(de_upper_bound) {}
};

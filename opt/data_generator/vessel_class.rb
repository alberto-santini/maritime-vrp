class VesselClass
  ...
  def initialize(options, name, capacity, cost_tc, draught, min_speed, max_speed, design_speed, cost_design_speed, cost_idle)
    ...
    if @options[:speeds_n] == 1
      @speeds << (@min_speed + @max_speed).to_f / 2.0
    else
      0.upto(@options[:speeds_n] - 1) do |sp|
        @speeds << @min_speed + sp * (@max_speed - @min_speed).to_f / (@options[:speeds_n] - 1)
      end
    end

class Port
  ...
  def to_json(*opt)
    "{
      ...
      \"total_movement_cost_pickup\": #{(@cost_per_container * (@is_hub ? 0 : @pickup_demand.to_i) / 1000.0).round(4)},
      \"total_movement_cost_delivery\": #{(@cost_per_container * (@is_hub ? 0 : @delivery_demand.to_i) / 1000.0).round(4)},
      \"call_fee_fixed_in_dollars\": #{(@call_fee_fixed / 1000.0).round(4)},
      \"call_fee_per_vessel_class_in_dollars\": #{@call_fee_vc.map{|c|(c / 1000.0).round(4)}.to_json},
      \"is_hub\": #{@is_hub},
      \"pickup_demand_in_ffe\": #{@is_hub ? 0 : @pickup_demand.to_i},
      \"delivery_demand_in_ffe\": #{@is_hub ? 0 : @delivery_demand.to_i},
      \"total_revenue_for_pickup\": #{(@pickup_revenue.to_f / 1000.0).round(4)},
      \"total_revenue_for_delivery\": #{(@delivery_revenue.to_f / 1000.0).round(4)},
      \"penalty_if_not_served_pickup\": #{(@penalty_if_not_served_pickup.to_f / 1000.0).round(4)},
      \"penalty_if_not_served_delivery\": #{(@penalty_if_not_served_delivery.to_f / 1000.0).round(4)},

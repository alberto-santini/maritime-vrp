...
public:
    ...
    std::vector<Solution> solve_elem_on_reduced_graph(float percentage) const;
    std::vector<Solution> solve_on_reduced_graph(const float percentage) const { return solve_on_generic_graph(percentage, false); }
...
private:
    ...
    std::vector<Solution> solve_on_generic_graph(float percentage, bool smart) const;
};

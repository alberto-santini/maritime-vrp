...
std::shared_ptr<Graph> GraphGenerator::create_graph(const ProblemData& data, std::shared_ptr<VesselClass> vessel_class, const bool remove_additional_arcs) {
    ...
    for(std::shared_ptr<Port> p : data.ports) {
        ...
        for(const auto& sc : vessel_class->bunker_cost) {
            ...
            auto final_time_pu = GraphGenerator::final_time(data, *p, arrival_time, PickupType::PICKUP);
            ...
            auto final_time_de = GraphGenerator::final_time(data, *p, arrival_time, PickupType::DELIVERY);
    ...
    for(std::shared_ptr<Port> p : data.ports) {
        ...
        auto earliest_arrival = GraphGenerator::earliest_arrival(data, p, n_h1.port, *vessel_class);
        auto latest_departure = GraphGenerator::latest_departure(data, p, n_h2.port, *vessel_class);
        
        for(auto t = 0; t < data.num_times - 1; t++) {
            ...
            for(auto pu : {PickupType::PICKUP, PickupType::DELIVERY}) {
                ...
                for(std::shared_ptr<Port> q : data.ports) {
                    ...
                    for(const auto& sc : vessel_class->bunker_cost) {
                        ...
                        auto final_time_pu = GraphGenerator::final_time(data, *q, arrival_time, PickupType::PICKUP);
                        ...
                        auto final_time_de = GraphGenerator::final_time(data, *q, arrival_time, PickupType::DELIVERY);
...
int GraphGenerator::final_time(const ProblemData& data, const Port& p, int arrival_time, PickupType pu) {
...
int GraphGenerator::latest_departure(const ProblemData& data, std::shared_ptr<Port> p, std::shared_ptr<Port> h2, const VesselClass& vessel_class) {
    auto top_speed = vessel_class.top_speed;
    return (data.num_times - floor(data.distances.at(std::make_pair(p, h2)) / top_speed));
}

int GraphGenerator::earliest_arrival(const ProblemData& data, std::shared_ptr<Port> p, std::shared_ptr<Port> h1, const VesselClass& vessel_class) {
    auto top_speed = vessel_class.top_speed;
    return ceil(data.distances.at(std::make_pair(h1, p)) / top_speed);
}

void GraphGenerator::create_edge(const Port& origin_p, PickupType origin_pu, int origin_type,
                                 const Port& destination_p, PickupType destination_pu, int destination_type,
                                 std::shared_ptr<Graph> g, float cost) {
